...
export function FloatingChat() {
  const { addTaskFromAi, fetchTasks } = useTasksContext();
  ...
  const abortActiveStream = useCallback((resetLoading = false) => {
    if (streamAbortRef.current) {
      streamAbortRef.current.abort();
      streamAbortRef.current = null;
    }
    if (resetLoading && isMountedRef.current) {
      setIsLoading(false);
  ...
  const handleSend = useCallback(async () => {
    ...
    streamAiChat(
      ...
      (actionName, args, payload = {}) => {
        ...
        setCurrentAction(null);
        if (actionName === "createTask" && payload.status === "success") {
          if (payload.task) {
            addTaskFromAi(payload.task);
          } else {
            fetchTasks();
          }
        }
  ...
  }, [input, isLoading, apiKey, model, messages, addTaskFromAi, fetchTasks]);
  ...
  const markdownComponents = {
    ...
    code: ({ inline, children, ...props }) =>
      inline ? (
        ...
        <pre
          className="rounded-lg p-3 text-[0.85em] overflow-x-auto"
              ...
              {isSettingsOpen ? (
                <div className="flex-1 p-5 flex flex-col gap-4 overflow-y-auto">
                  ...
                  >
                    ...
                    của bạn (LocalStorage). Chúng tôi{" "}
                    <strong>không bao giờ</strong> gửi hay lưu key lên server.
                  </div>
