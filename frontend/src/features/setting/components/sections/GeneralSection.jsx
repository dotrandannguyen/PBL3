...
const TIMEZONES = [
  {
    ...
    hint: "Ho Chi Minh, Hanoi",
  },
  {
    value: "Asia/Tokyo",
    label: "(GMT+9) Japan Standard",
    hint: "Tokyo, Osaka",
  },
  ...
  return (
    ...
    >
      <div className="flex items-center gap-1">
        <div
          className={`h-1.5 w-1.5 rounded-full ${isLight ? "bg-red-400" : "bg-red-500"}`}
        />
        <div
          className={`h-1.5 w-1.5 rounded-full ${isLight ? "bg-yellow-400" : "bg-yellow-500"}`}
        />
        <div
          className={`h-1.5 w-1.5 rounded-full ${isLight ? "bg-green-400" : "bg-green-500"}`}
        />
      </div>
      <div className="flex flex-1 gap-1.5">
        <div
          className={`h-full w-7 rounded-sm ${isLight ? "bg-[#e8e8e6]" : "bg-[#202020]"}`}
        />
        <div className="flex flex-1 flex-col gap-1 pt-0.5">
          <div
            className={`h-1 w-3/4 rounded-full ${isLight ? "bg-[#c4c4c0]" : "bg-[#484848]"}`}
          />
          <div
            className={`h-1 w-2/4 rounded-full ${isLight ? "bg-[#d0d0cc]" : "bg-[#3a3a3a]"}`}
          />
          <div
            className={`h-1 w-3/5 rounded-full ${isLight ? "bg-[#d0d0cc]" : "bg-[#3a3a3a]"}`}
          />
        </div>
      </div>
    </div>
...
const GeneralSection = () => {
  ...
  const themeOptions = [
    ...
    { value: "dark", icon: Moon, label: t("general.theme.dark") || "Dark" },
    {
      value: "system",
      icon: Monitor,
      label: t("general.theme.default") || "System",
    },
  ];
  ...
  const handleTimeFormatChange = async (value) => {
    ...
    if (!user?.id) return;
    try {
      const response = await updateCurrentUser({ timeFormat: value });
      const updated = response?.data?.data;
      if (updated) updateUserInStorage(updated);
    } catch (error) {
      console.warn("Không thể lưu time format lên server", error);
    }
  };
  ...
  const handleTimezoneChange = async (value) => {
    ...
    if (!user?.id) return;
    try {
      const response = await updateCurrentUser({ timezone: value });
      const updated = response?.data?.data;
      if (updated) updateUserInStorage(updated);
    } catch (error) {
      console.warn("Không thể lưu timezone lên server", error);
    }
  };
